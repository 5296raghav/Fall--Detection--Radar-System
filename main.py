...
class MainClass:
    def __init__(self):
        ...
        self.radar_fs = self.config.getfloat('radar_fs', 200.0)
        ...
        self.radarThread = CollectionThreadX4MP(1,'radarThreadX4',self.radarStopEventMP, radarSettings=self.radarSettings, baseband=True, dataQueue=self.radarDataQMP)
        ...
        self.radarDataDeck = collections.deque(maxlen=int(14000))
    ...
    def main(self):
        ...
        while True:
             if not self.radarDataQMP.empty():               
                 ...
                 while not self.radarDataQMP.empty():
                      #self.radarDeck.append(self.radarDataQMP.get())
                 ...
                 #arr_len=array([radarDataDeck])
                 #print(len(arr_len))
                 #queue=deque([self.radarDataDeck])
    ...
    def cleanData(self):
        ...
        timeString = time.strftime(u"%Y%m%d-%H%M%S")
        #data1 = pd.read_csv(self.radar_data_dir + timeString + '_' + self.radar_file_name + '_.csv') 
        data1 = pd.read_csv('20190817-162913_radar_data_.csv')
	data1.dtypes
